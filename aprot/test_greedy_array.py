...
class Composite(aprot.struct):
    __metaclass__ = aprot.struct_generator
    _descriptor = [("x", aprot.u16),
                   ("y", aprot.i16)]

class ComplexComposite(aprot.struct):
    __metaclass__ = aprot.struct_generator
    _descriptor = [("y_len", aprot.u32),
                   ("x", Composite),
                   ("y", aprot.array(Composite, bound = "y_len"))]

class TestGreedyScalarArray():

    class Array(aprot.struct):
        __metaclass__ = aprot.struct_generator
        _descriptor = [("x", aprot.array(aprot.u32))]
...
class TestGreedyCompositeArray():
    ...
    class Array(aprot.struct):
        __metaclass__ = aprot.struct_generator
        _descriptor = [("x", aprot.array(Composite))]
...
class TestGreedyComplexCompositeArray():
    ...
    class Array(aprot.struct):
        __metaclass__ = aprot.struct_generator
        _descriptor = [("x", aprot.array(ComplexComposite))]
...
def test_exceptions():
    with pytest.raises(Exception):
        class NotLast(aprot.struct):
            __metaclass__ = aprot.struct_generator
            _descriptor = [("y", aprot.array(aprot.u32)),
                           ("x", aprot.u32)]
    with pytest.raises(Exception):
        class GreedyComposite(aprot.struct):
            __metaclass__ = aprot.struct_generator
            _descriptor = [("x", aprot.array(aprot.u32))]
        class GreedyArrayOfGreedyComposites(aprot.struct):
            __metaclass__ = aprot.struct_generator
            _descriptor = [("x", aprot.array(GreedyComposite))]
