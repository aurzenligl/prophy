...
def test_struct_with_multiple_dynamic_fields():
    ...
    assert b'\x00\x02\x02\x00\x00\x00\x00\x01\x00\x00\x00\x02\x03\x04' == x.encode('>')
    assert b'\x02\x00\x02\x00\x01\x00\x00\x00\x02\x00\x00\x00\x03\x04' == x.encode('<')
...
def test_struct_with_greedy_bytes():
    ...
    assert b'\x00\x03\x00\x05\x00\x06\x00\x07ala ma kota' == x.encode('>')
    assert b'\x03\x00\x05\x00\x06\x00\x07\x00ala ma kota' == x.encode('<')

    x.decode(b'\x00\x01\x00\x08abecadlo', '>')
    assert [8] == x.a[:]
    assert b'abecadlo' == x.b
