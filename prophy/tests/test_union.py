...
def test_union_encode_according_to_largest_field(VariableLengthFieldsUnion):
    ...
    x.discriminator = "a"
    x.a = 0x12
    assert "\x00\x00\x00\x00\x00\x00\x00\x00" "\x12\x00\x00\x00\x00\x00\x00\x00" == x.encode(">")
    assert "\x00\x00\x00\x00\x00\x00\x00\x00" "\x12\x00\x00\x00\x00\x00\x00\x00" == x.encode("<")

    x.discriminator = "b"
    x.b = 0x1234
    assert "\x00\x00\x00\x01\x00\x00\x00\x00" "\x12\x34\x00\x00\x00\x00\x00\x00" == x.encode(">")
    assert "\x01\x00\x00\x00\x00\x00\x00\x00" "\x34\x12\x00\x00\x00\x00\x00\x00" == x.encode("<")

    x.discriminator = "c"
    x.c = 0x12345678
    assert "\x00\x00\x00\x02\x00\x00\x00\x00" "\x12\x34\x56\x78\x00\x00\x00\x00" == x.encode(">")
    assert "\x02\x00\x00\x00\x00\x00\x00\x00" "\x78\x56\x34\x12\x00\x00\x00\x00" == x.encode("<")

    x.discriminator = "d"
    x.d = 0x123456789ABCDEF1
    assert "\x00\x00\x00\x03\x00\x00\x00\x00" "\x12\x34\x56\x78\x9a\xbc\xde\xf1" == x.encode(">")
    assert "\x03\x00\x00\x00\x00\x00\x00\x00" "\xf1\xde\xbc\x9a\x78\x56\x34\x12" == x.encode("<")

def test_union_decode_according_to_largest_field(VariableLengthFieldsUnion):
    x = VariableLengthFieldsUnion()

    assert 16 == x.decode("\x00\x00\x00\x00\x00\x00\x00\x00" "\x12\x00\x00\x00\x00\x00\x00\x00", ">")
    assert 0 == x.discriminator
    assert 0x12 == x.a
    ...
    assert 16 == x.decode("\x00\x00\x00\x01\x00\x00\x00\x00" "\x12\x34\x00\x00\x00\x00\x00\x00", ">")
    assert 1 == x.discriminator
    assert 0x1234 == x.b

    assert 16 == x.decode("\x01\x00\x00\x00\x00\x00\x00\x00" "\x34\x12\x00\x00\x00\x00\x00\x00", "<")
    assert 1 == x.discriminator
    assert 0x1234 == x.b

    assert 16 == x.decode("\x00\x00\x00\x02\x00\x00\x00\x00" "\x12\x34\x56\x78\x00\x00\x00\x00", ">")
    assert 2 == x.discriminator
    assert 0x12345678 == x.c

    assert 16 == x.decode("\x02\x00\x00\x00\x00\x00\x00\x00" "\x78\x56\x34\x12\x00\x00\x00\x00", "<")
    assert 2 == x.discriminator
    assert 0x12345678 == x.c

    assert 16 == x.decode("\x00\x00\x00\x03\x00\x00\x00\x00" "\x12\x34\x56\x78\x9a\xbc\xde\xf1", ">")
    assert 3 == x.discriminator
    assert 0x123456789ABCDEF1 == x.d

    assert 16 == x.decode("\x03\x00\x00\x00\x00\x00\x00\x00" "\xf1\xde\xbc\x9a\x78\x56\x34\x12", "<")
    assert 3 == x.discriminator
    assert 0x123456789ABCDEF1 == x.d
...
def test_union_decode_exceptions(VariableLengthFieldsUnion):
    ...
    with pytest.raises(Exception) as e:
        x.decode("\x00\x00\x00\x02\x00\x00\x00\x00" "\x12\x34\x56\x78\x00\x00\x00\x00\x00", ">")
    assert "not all bytes read" == e.value.message

    with pytest.raises(Exception) as e:
        x.decode("\x00\x00\x00\x02\x00\x00\x00\x00" "\x12\x34\x56\x78\x00\x00\x00", ">")
...
def test_array_with_union():
    ...
    assert "\x03\x00\x00\x00""\x00\x00\x00\x00""\x00\x01\x00\x00""\x00\x00\x00\x01""\x02\x00\x00\x00""\x00\x00\x00\x02""\x03\x00\x00\x00" == x.encode(">")
    assert "\x03\x00\x00\x00""\x00\x00\x00\x00""\x01\x00\x00\x00""\x01\x00\x00\x00""\x02\x00\x00\x00""\x02\x00\x00\x00""\x03\x00\x00\x00" == x.encode("<")

    x.decode("\x02\x00\x00\x00""\x00\x00\x00\x01""\x01\x00\x00\x00""\x00\x00\x00\x02""\x02\x00\x00\x00", ">")
...
def test_union_with_plain_struct():
    ...
    assert "\x00\x00\x00\x01\x02\x03\x00\x00" == x.encode(">")

    x.decode("\x00\x00\x00\x01\x06\x07\x00\x00", ">")
    assert 1 == x.discriminator
    assert 6 == x.b.a
    assert 7 == x.b.b
...
def test_union_with_struct_with_array_and_bytes():
    ...
    assert "\x00\x00\x00\x00" "abc\x00" == x.encode(">")

    x.discriminator = 1
    x.b.a[0].a = 3
    x.b.a[1].a = 4
    x.b.a[2].a = 5
    assert "\x00\x00\x00\x01" "\x03\x04\x05\x00" == x.encode(">")

    x.decode("\x00\x00\x00\x01" "\x07\x08\x09\x00", ">")
    assert 1 == x.discriminator
    assert 7 == x.b.a[0].a
    assert 8 == x.b.a[1].a
    assert 9 == x.b.a[2].a
...
def test_union_with_nested_struct_and_union():
    ...
    x.a.discriminator = 1
    x.a.b = 0xFFF
    assert "\x00\x00\x00\x00" "\x00\x00\x00\x01" "\x0f\xff\x00\x00" == x.encode(">")

    x = U()
    x.discriminator = 1
    x.b.a.a = 0xF
    assert "\x00\x00\x00\x01" "\x0f\x00\x00\x00\x00\x00\x00\x00" == x.encode(">")

    x.decode("\x00\x00\x00\x00" "\x00\x00\x00\x01" "\x00\x08\x00\x00", ">")
    assert 8 == x.a.b
...
def test_union_with_static_bytes():
    ...
    x.decode("\x00\x00\x00\x00" "\x01\x02\x03\x00", "<")
